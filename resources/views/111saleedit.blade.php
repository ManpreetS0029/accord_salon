...
@section('content')
    ...
    <div id="display_screen" class="container">
        <div class="row">
         ...
         <div class="panel-heading">Update Sale <div style="float:right">
<a href="<?php echo route('sale.index'); ?>">List Sales</a> | 
         <a href="<?php echo URL::to('/'); ?>/sale/<?php echo $sale->id; ?>/paymenthistory">View Payment History <?php if( ( $sale->getTotalPaidAmount() + $sale->getAmountUnderReview() ) < $sale->paidprice) { echo ' | <a href="'.route('sale.addpayment',$sale->id).'">Add Payment</a>'; } ?></a></div></div>
                                ...
                                <div class="row row-lg">
                                    ...
                                    <div class="col-lg-6">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            {{ Form::select('clientid', $clients, $sale->client ? $sale->client->id : '0' , array('class' => 'form-control', 'data-plugin' => "select2") ) }}
                                        </div>
...
?>
					...
					<div class="row">
                        ...
                        $byStaff .= '<div class="service_staff_box" data-name="staffs_'.$item->itemid.'_'.$packageItem->itemid.'">'.$packageItem->title.'<button type="button" class="addstaff">Add Staff</button>';
...
$staffOptionsIn = '<option value="">Select</option>';
                        ...
                        $byStaff .= '<div class="service_done_assign"><select class="form-control" name="staffs_'.$item->itemid.'_'.$packageItem->itemid.'[]">'.$staffOptionsIn.'</select><div class="row"><div class="col-md-4"><button type="button" class="remove_service_done"><i class="fa fa-remove"></i></button></div></div></div>';
                        
                        //					$byStaff .= '<div>'.$packageItem->title.': <select class="form-control" name="staffs_'.$item->itemid.'_'.$packageItem->itemid.'">'.$staffOptionsIn.'</select></div>';
						}
                        $byStaff .= '</div>';
                        }
						}
                        ...
                        $byStaff .= '<div class="service_staff_box" data-name="staffs_services_'.$item->itemid.'"><button type="button" class="addstaff">Add Staff</button>';
                        ...
                        $byStaff .= '<div class="service_done_assign"><select class="form-control" name="staffs_services_'.$item->itemid.'[]">'.$staffOptionsIn.'</select><div class="row"><div class="col-md-4"><button type="button" class="remove_service_done"><i class="fa fa-remove"></i></button></div></div></div>';
                        }
                            $byStaff .= '</div>';
                        ...
                        $byStaff .= '<div class="service_staff_box" data-name="staffs_product_'.$item->itemid.'"><button type="button" class="addstaff">Add Staff</button>';
                        foreach( $item->doneByStaffMembers as $staffMain ) { 
                        $staffOptionsIn = '<option value="">Select</option>';    
                        foreach ( $staffs as $staff )
						{
						...
						$staffOptionsIn .= '<option '.$selected.' value="'.$staff->id.'">'.$staff->firstname.' '.$staff->lastname.'</option>';
                                                ...
                                                $byStaff .= '<div class="service_done_assign"><select class="form-control" name="staffs_product_'.$item->itemid.'[]">'.$staffOptionsIn.'</select><div class="row"><div class="col-md-4"><button type="button" class="remove_service_done"><i class="fa fa-remove"></i></button></div></div></div>';
						...
						$rowsString = '<tr>';
						$rowsString .= '<td>'.$k.'<input type="hidden" data-type="'.$item->itemtype.'" class="sale_ids" name="ids[]" value="'.$item->itemid.'" /><input type="hidden" class="sale_itemtype" name="itemtypes[]" value="'.$item->itemtype.'" /></td>';
						$rowsString .= '<td>'.$item->title.'('.$item->barcode.')</td>';
