...
var pCount = 1;
	...
	$(document).on("click",".addbtnssale", function(e) {
		...
		for (var x = 0; x < jsonObjForServices.length; x++) {
			...
			var newString = rowsString.replace("{rowno}",pCount);
			...
			if( jsonObjForServices[x]['types'] == 'package' )
			{
				...
				{
				    strStaff += '<div>'+jsonObjForServices[x]['services'][k]['name']+': <select name="staffs_'+jsonObjForServices[x]["id"]+'_'+jsonObjForServices[x]['services'][k]["id"]+'" class="form-control">'+staffOptions+'</select></div>';
				}
			    }
			}
			else if( jsonObjForServices[x]['types'] == 'service' )
			{
			    strStaff += '<div><select name="staffs_services_'+jsonObjForServices[x]["id"]+'" class="form-control">'+staffOptions+'</select></div>';
			}
	...
	$("#invoicetable").find("tr").each(function(index, element) {
	    ...
	    var unitprice = parseFloat($(element).find(".class_unitprice").text());
	...
	{
	    discountAmount = globalDiscountValue * subTotal / 100;
	...
	$("#taxable_amount").html( roundTo(subTotal - discountAmount,2));
	$("#taxamount").html( roundTo(tax, 2 ) );
	$("#grandtotal").html( roundTo(subTotal - discountAmount + tax,2) );
    }
